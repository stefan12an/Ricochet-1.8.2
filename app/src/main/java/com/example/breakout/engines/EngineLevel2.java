...
public class EngineLevel2 extends SurfaceView implements Runnable {
    ...
    Drawable drawable4 = getResources().getDrawable(R.drawable.brick21);
    ...
    public EngineLevel2(Context context, int x, int y) {
        super(context);
        ourHolder = getHolder();
        paint = new Paint();
        screenX = x;
        ...
        bgMusic=MediaPlayer.create(context,R.raw.musiclvl2);
        ...
        if (ourHolder.getSurface().isValid()) {
            ...
            for (int i = 0; i < numBricks; i++)
            {
                if (bricks[i].getVisibility())
                {
                    ...
                    if (i%2!=0)
                    {
                        if(bricks[i].getLives()==1)
                        {
                            drawable4.setBounds(bricks[i].getRect1());
